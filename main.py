...
async def endgvg(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("\ud83d\udccb GVG log is being generated...", ephemeral=True)
    try:
        await finalize_log()
        print("\u2705 GVG tracking ended and log sent.")
    except Exception as e:
        print(f"\u274c Error in finalize_log: {e}")
...
# Flask ping server to keep Replit alive
app = Flask('')
